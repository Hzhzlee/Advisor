  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-4 bg-slate-50 border-b border-slate-200">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-600 mt-0.5">
              Live audit stream of all client-to-server MCP tool invocations through <span className="font-mono text-slate-800 font-medium">/api/mcp</span>.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          {/* Tab Switch */}
          <div className="flex items-center p-1 bg-white rounded-lg border border-slate-200 text-xs shadow-xs">
            <button
              type="button"
              ...
              return (
                <div key={log.id} className="p-3.5 hover:bg-slate-50/80 transition-colors">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 text-xs font-mono text-slate-500 pl-6 sm:pl-0">
                  ...
                  {log.args && Object.keys(log.args).length > 0 && !isExpanded && (
                    <div className="mt-2 pl-6 sm:pl-7 text-[11px] font-mono text-slate-500 flex items-center gap-2 overflow-hidden text-ellipsis whitespace-nowrap">
                  ...
                  {isExpanded && (
                    <div className="mt-3 pl-6 sm:pl-7 pt-2 border-t border-slate-100 space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          ...
                          className="flex items-center gap-1 text-[10px] font-mono text-slate-700 hover:text-slate-900 bg-slate-100 border border-slate-200 px-2 py-0.5 rounded transition-colors"
                        >
                          {copiedLogId === log.id ? (
                            <>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-[11px] font-mono">
                        {/* Request Box */}
                        <div className="rounded-lg bg-slate-950 border border-slate-800 p-3 overflow-x-auto text-slate-200 shadow-inner">
          ...
          {registeredTools.map((t) => (
            <div key={t.name} className="p-3.5 rounded-lg bg-slate-50 border border-slate-200">
              <div className="flex items-center justify-between pb-2 border-b border-slate-200">
                ...
                <span className="text-[10px] text-slate-500 uppercase font-semibold">MCP Tool</span>
              </div>
              <p className="text-xs text-slate-700 font-sans mt-2">{t.description}</p>
              <div className="mt-3 text-[10px] text-slate-700 bg-white p-2.5 rounded border border-slate-200">
                <span className="text-slate-500 font-semibold">Input Schema:</span>
      ...
      {/* Footer Info */}
      <div className="px-5 py-2.5 bg-slate-50 border-t border-slate-200 flex items-center justify-between text-[11px] text-slate-600 font-mono">
