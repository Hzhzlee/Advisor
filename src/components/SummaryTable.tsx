    ...
    <section id="summary" className="rounded-xl border border-slate-200 bg-white p-5 shadow-xs">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4">
        ...
        <div className="flex items-center gap-3 text-xs text-slate-500 font-mono">
          ...
          <span aria-hidden="true">·</span>
          <span className="text-slate-900 font-bold">Total Invested: {sym}{totalContributed.toLocaleString()}</span>
        </div>
      </div>
      ...
      <div className="overflow-x-auto rounded-lg border border-slate-200 bg-white">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-100 text-xs font-mono tabular-nums">
              ...
              return (
                <tr
                  ...
                  className={`hover:bg-slate-50 transition-colors ${
                    isBase ? 'bg-blue-50/60 font-semibold text-slate-900' : 'text-slate-800'
                  }`}
                >
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-2 font-sans font-medium text-slate-900">
                      <span
                      ...
                      <span>{s.name}</span>
                      {isBase && (
                        <span className="text-[10px] font-mono text-blue-700 font-bold ml-1">
                          (Baseline Anchor)
                        </span>
                      )}
                    </div>
                  ...
                  <td className={`py-3 px-4 text-right font-bold ${isBase ? 'text-blue-900 text-sm' : 'text-slate-900'}`}>
                    {formatCurrency(s.final_value)}
                  </td>
                  <td className="py-3 px-4 text-right text-slate-600">
                    {formatCurrency(s.total_contributed)}
                  </td>
                  <td className={`py-3 px-4 text-right font-bold ${isGainPositive ? 'text-emerald-700' : 'text-rose-600'}`}>
            ...
            {monteCarlo && (
              <>
                <tr className="border-t border-slate-200 bg-emerald-50/40 text-slate-800">
                  ...
                  <td className="py-2.5 px-4 text-right text-slate-600">Stochastic</td>
                  <td className="py-2.5 px-4 text-right font-bold text-emerald-800">
                    {formatCurrency(monteCarlo.p90_final)}
                  </td>
                  <td className="py-2.5 px-4 text-right text-slate-600">{formatCurrency(totalContributed)}</td>
                  <td className="py-2.5 px-4 text-right text-emerald-800 font-bold">
                ...
                <tr className="bg-sky-50/40 text-slate-800">
                  <td className="py-2.5 px-4 font-sans font-medium text-slate-900">
                    <div className="flex items-center gap-2">
                  ...
                  <td className="py-2.5 px-4 text-right text-slate-600">Stochastic</td>
                  <td className="py-2.5 px-4 text-right font-bold text-sky-800">
                    {formatCurrency(monteCarlo.p50_final)}
                  </td>
                  <td className="py-2.5 px-4 text-right text-slate-600">{formatCurrency(totalContributed)}</td>
                  <td className="py-2.5 px-4 text-right text-sky-800 font-bold">
                ...
                <tr className="bg-rose-50/40 text-slate-800">
                  <td className="py-2.5 px-4 font-sans font-medium text-slate-900">
                    <div className="flex items-center gap-2">
                  ...
                  <td className="py-2.5 px-4 text-right text-slate-600">{formatCurrency(totalContributed)}</td>
                  <td className={`py-2.5 px-4 text-right font-bold ${monteCarlo.p10_final >= totalContributed ? 'text-emerald-800' : 'text-rose-700'}`}>
      ...
      <div className="mt-4 p-3.5 rounded-lg bg-slate-50 border border-slate-200 text-xs text-slate-600 space-y-1.5">
        ...
        <ul className="list-disc list-inside space-y-1 text-[11px] text-slate-600 pl-1">
          <li>
            <strong className="text-slate-800">Monthly Compounding:</strong> Contributions occur at the beginning of each calendar month and compound monthly using effective rate <span className="font-mono text-slate-800 bg-slate-200/70 px-1 py-0.5 rounded">(1 + r)^(1/12) - 1</span>.
          </li>
          <li>
